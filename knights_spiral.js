...
var knights_spiral = {
    ...
    // Decode gunn.gif early so LUDICROUS plaid does not flash the
    // procedural fallback on first frames (async race with draw_plaid).
    _preload_plaid: function() {
        var self = this;
        var img  = new Image();
        img.onload = function() {
            self._plaid_img = img;
    ...
    // Build a side×side export canvas (1 cell = 1 pixel). Uses the
    // offscreen pixel snapshot when available; otherwise paints from
    // this.painted in sorted key order for a stable fallback.
    board_png_canvas: function() {
        this._pixel_off_ensure();
        if (this._pixel_off && this._pixel_off_half === this.half) {
            return this._pixel_off;
        }
        var side = this.side;
        var half = this.half;
        var c    = document.createElement("canvas");
        ...
        cx.fillRect(0, 0, side, side);
        var colors = this.PIXEL_COLORS;
        var keys   = Object.keys(this.painted).sort();
        for (var i = 0; i < keys.length; i++) {
            var parts = keys[i].split(",");
            var bx    = parseInt(parts[0], 10);
            var by    = parseInt(parts[1], 10);
            cx.fillStyle = colors[this.painted[keys[i]]];
            cx.fillRect(half + bx, half + by, 1, 1);
        }
        return c;
    },

    // Download the finished board as a PNG. Exports the logical
    // side×side pixel board (not the scaled viewport canvas) so
    // identical URL parameters always yield identical pixels.
    save_image: function() {
        var self = this;
        var side = this.side;
        var name = "knights_spiral-" + side + "-" + this.moves_done +
                   "moves.png";
        var src  = this.board_png_canvas();
